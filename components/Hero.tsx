...
import { ANDROID_DOWNLOAD_LINK, IOS_DOWNLOAD_LINK } from '../downloadConfig';
...
const KEYWORDS = "esports app, bgmi tournament app, free fire earning app, play games earn money, esports india, cod mobile tournaments, pubg mobile lite tournament app";

// --- Memoized Components for Performance (Prevents re-renders) ---
...
const Hero: React.FC = () => {
  // Performance: Default to "India" to avoid UI flicker while fetching
  const [userCountry, setUserCountry] = useState<string>("India");

  useEffect(() => {
    // Controller to cancel fetch if component unmounts (Memory Leak Fix)
    const controller = new AbortController();
    
    // Low priority fetch (doesn't block main thread)
    const fetchCountry = async () => {
      try {
        ...
        // Silently fail to default "India" - no console spam for users
      }
    };
  ...
  const handleDownload = (link: string | undefined, platform: string) => {
    if (link) {
  ...
  // --- JSON-LD Schema: This makes Google show "App Rating" & "Stars" in Search ---
  const schemaMarkup = {
  ...
  return (
    <>
      ...
      <Helmet>
        ...
        <meta property="og:image" content="https://images.unsplash.com/photo-1542751371-adc38448a05e?auto=format&fit=crop&q=80&w=800" />
        
        {/* Twitter Card */}
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content="Play Esports & Win Big" />
        <meta name="twitter:image" content="https://images.unsplash.com/photo-1542751371-adc38448a05e?auto=format&fit=crop&q=80&w=800" />
      ...
      <section id="home" className="relative w-full min-h-screen pt-32 pb-20 flex items-center overflow-hidden bg-gaming-bg">
        
        {/* Performance: Use CSS-only background patterns (GPU Accelerated) */}
        <div className="absolute inset-0 z-0 pointer-events-none transform-gpu">
          <div className="absolute top-[-10%] left-[-10%] w-[60%] h-[60%] bg-gaming-primary/15 rounded-full blur-[120px] opacity-40 will-change-transform"></div>
          <div className="absolute bottom-[-10%] right-[-10%] w-[60%] h-[60%] bg-gaming-secondary/15 rounded-full blur-[120px] opacity-40 will-change-transform"></div>
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8 z-10 relative grid lg:grid-cols-2 gap-12 lg:gap-8 items-center">
          
          {/* Left Content: Using Sematic HTML (article/h1/h2) for SEO */}
          <article className="text-center lg:text-left order-2 lg:order-1">
            <div className="inline-flex items-center px-4 py-1.5 mb-6 border border-gaming-secondary/30 rounded-full bg-gaming-secondary/10 backdrop-blur-md shadow-[0_0_15px_rgba(14,165,233,0.2)]">
              <Sparkles className="w-3.5 h-3.5 text-gaming-secondary mr-2 animate-pulse" aria-hidden="true" />
              <span className="text-gaming-secondary font-display font-bold tracking-widest text-xs uppercase">{userCountry}'s #1 Esports Arena</span>
            </div>
            
            <h1 className="text-4xl md:text-6xl lg:text-7xl font-display font-bold text-white mb-6 leading-[1.1] tracking-tight drop-shadow-2xl">
              PLAY. <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-gray-100 via-gray-300 to-gray-500">COMPETE.</span> <br />
              {/* Performance: Ensure high contrast for Accessibility */}
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-gaming-secondary via-gaming-primary to-gaming-accent animate-shimmer bg-[length:200%_100%]">
                WIN BIG.
              </span>
            </h1>
            ...
            <div className="flex flex-col items-center lg:items-start space-y-6">
              <div className="flex flex-col sm:flex-row gap-4 w-full max-w-md">
                ...
                {/* Android Button: Added aria-label for Screen Readers */}
                <div className="relative group flex-1 transform transition-transform hover:scale-105 active:scale-95">
                  <div className="absolute -inset-0.5 bg-gradient-to-r from-gaming-primary to-gaming-secondary rounded-xl blur opacity-40 group-hover:opacity-70 transition duration-200"></div>
                  <button 
              ...
              <div className="flex items-center justify-center lg:justify-start gap-4 text-xs md:text-sm text-gray-500 font-medium w-full">
                ...
                <div className="flex items-center bg-white/5 px-3 py-1 rounded-lg border border-white/5">
                  <Shield className="w-3.5 h-3.5 mr-2 text-gaming-secondary" aria-hidden="true" />
                  100% Secure
                </div>
              </div>
            </div>
          ...
          {/* Right Side - Live Hub */}
          <div className="relative flex items-center justify-center order-1 lg:order-2 py-10 lg:py-0">
            <div className="relative w-full max-w-sm aspect-square flex flex-col items-center justify-center transform-gpu">
              
              <div className="absolute inset-0 bg-gradient-to-b from-gaming-primary/20 to-gaming-secondary/10 rounded-[3rem] blur-3xl"></div>
              
              <div className="relative w-full bg-[#0a0a0f]/80 backdrop-blur-xl border border-white/10 rounded-[2.5rem] p-6 shadow-[0_0_50px_rgba(0,0,0,0.5)] overflow-hidden group flex flex-col justify-between min-h-[380px]">
                
                {/* Performance Optimization: 
                   1. fetchpriority="high" - tells browser to load this image ASAP (LCP).
                   2. width/height - prevents Cumulative Layout Shift (CLS).
                */}
                <div className="absolute inset-0 opacity-30 group-hover:opacity-40 transition-opacity duration-500">
                  ...
                  />
                  <div className="absolute inset-0 bg-gradient-to-b from-[#0a0a0f]/40 via-[#0a0a0f]/90 to-[#0a0a0f]"></div>
                </div>

                {/* Dashboard Content */}
                <div className="relative flex justify-between items-center z-10">
                  <div className="flex items-center gap-2 bg-red-500/10 px-3 py-1 rounded-full border border-red-500/20 backdrop-blur-sm">
                    <span className="relative flex h-2 w-2">
                      <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-red-400 opacity-75"></span>
                      <span className="relative inline-flex rounded-full h-2 w-2 bg-red-500"></span>
                    </span>
                    <span className="text-red-500 text-[10px] font-bold tracking-widest uppercase">Live Hub</span>
                  </div>
                  <Target className="text-white/30 w-5 h-5" aria-hidden="true" />
                </div>

                <div className="relative z-10 flex-1 flex flex-col justify-center items-center py-4">
                  <div className="text-center mb-6">
                    <div className="inline-flex items-center justify-center p-2.5 bg-yellow-400/10 rounded-2xl mb-3 border border-yellow-400/20 shadow-[0_0_15px_rgba(250,204,21,0.15)]">
                      <Trophy className="w-8 h-8 text-yellow-400 fill-yellow-400/20" aria-hidden="true" />
                    </div>
                    <p className="text-gray-400 text-[10px] uppercase tracking-[0.3em] font-bold mb-1">Weekly Prize Pool</p>
                    <h2 className="text-5xl font-display font-bold text-transparent bg-clip-text bg-gradient-to-b from-white to-gray-400 drop-shadow-lg">
                      ₹50k
                    </h2>
                  </div>

                  <div className="grid grid-cols-2 gap-3 w-full">
                    <div className="bg-[#1a1a20]/60 p-3 rounded-xl border border-white/5 text-center backdrop-blur-sm hover:bg-[#1a1a20]/80 transition-colors">
                      ...
                      <p className="text-[9px] text-gray-500 uppercase font-bold tracking-wider">Live Players</p>
                    </div>
                    ...
                    <div className="bg-[#1a1a20]/60 p-3 rounded-xl border border-white/5 text-center backdrop-blur-sm hover:bg-[#1a1a20]/80 transition-colors">
                      ...
                      <p className="text-[9px] text-gray-500 uppercase font-bold tracking-wider">Leagues</p>
                    </div>
                  </div>
                </div>

                <div className="relative z-10 mt-auto pt-4">
                  <div className="w-full h-0.5 bg-gradient-to-r from-transparent via-white/10 to-transparent mb-3"></div>
                  <div className="flex justify-between items-center text-[11px] text-gray-400 font-medium">
                    <span>Next Match:</span>
                    <span className="text-gaming-secondary font-bold">Starts in 04:59</span>
                  </div>
                </div>
              </div>
              
              <div className="absolute bottom-6 -left-4 md:-left-8 w-auto px-4 py-3 bg-[#111]/90 rounded-xl border border-white/10 backdrop-blur-xl flex items-center gap-3 animate-float delay-300 shadow-[0_10px_30px_rgba(0,0,0,0.5)] z-20">
                <span className="relative flex h-2.5 w-2.5">
                  <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75"></span>
                  <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-green-500"></span>
                </span>
                <div className="flex flex-col text-left">
                  <span className="text-[9px] text-gray-500 uppercase leading-none font-bold mb-1 tracking-wider">Online Now</span>
                  <span className="text-base font-bold text-white leading-none font-display">18,249</span>
                </div>
              </div>
            </div>
          </div>
